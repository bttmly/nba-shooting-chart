...
(function() {
  var TwoDimensionalArray, collectify, getJSON, makeLeagueAverageBins, makePlayerBins, root, toBins,
  ...
  if (typeof WorkerGlobalScope !== 'undefined' && self instanceof WorkerGlobalScope) {
    self.addEventListener("message", function(event) {
      if (event.data.cmd === "stop") {
        ...
        return makeLeagueAverageBins(event.data.msg);
      }
    });
  } else {
    root = exports || window;
  ...
  makeLeagueAverageBins = function(msg) {
    return getJSON(msg.ajaxData, function(data) {
      var binOpts, cleanShooting, minimalBins, pctBins, postMsg, rawShootingCollection, shotBins, threshold;
      ...
      return self.postMessage(JSON.stringify(postMsg));
    }, function(req, status) {
      throw new Error("Server responded with status " + status);
    });
  };

  makePlayerBins = function(playerId) {};
